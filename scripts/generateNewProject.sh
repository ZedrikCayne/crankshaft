...
if [ -d ${1} ]; then
    echo "Directory ${1} exists already. Stopping."
    exit 1;
fi
...
target_include_directories( ${1} PUBLIC crankshaft/include )

target_link_directories( ${1} PUBLIC crankshaft/lib )

if( MACOSX )
if( OPENSSL_FOUND )
target_include_directories( ${1} PUBLIC \${OPENSSL_INCLUDE_DIR} )
target_link_libraries( ${1} \${OPENSSL_LIBRARIES} )
target_link_libraries( ${1} -lcrankshaft )
endif()
else()
target_link_libraries( ${1} m ssl crypto pthread m z dl crankshaft )
endif()
EOT

echo "Initializing git repo with submodule."
pushd ${1}
git init
git submodule add https://github.com/ZedrikCayne/crankshaft.git crankshaft
popd
...
chmod u+x ${1}/build.sh

echo "Creating run script."

cat <<EOT > ${1}/run.sh
build/${1} --info --port 8080
EOT
...
echo "Stubbing in main.cpp"
cat <<EOT > ${1}/src/main.cpp
#include <stdio.h>
...
static char defaultServerName[] = "${1}";
...
void PrintHeader() {
    ...
    printf("${1} built with crankshaft\\n");
}
