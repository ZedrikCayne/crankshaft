...
static struct CS_List *privateTestListPointersToInt(int32_t nItems) {
    struct CS_List *testList = CS_listCreate(1024);
    if( testList ) {
        for( int32_t i = 0; i < nItems; ++i ) {
            CS_listPushTail( testList, &i, sizeof(int32_t) );
        }
    }
    return testList;
}

static struct CS_List *privateTestListVoidsOfInt(int32_t nItems) {
    struct CS_List *testList = CS_listCreate(1024);
    if( testList ) {
        for( int32_t i = 0; i < nItems; ++i ) {
            CS_listPushTail( testList, (void*)(int32_t)i, 0 );
        }
    }
    return testList;
}

static int32_t ITEM_VAL(const struct  CS_ListItem *item) {
    if( item == NULL ) return -1;

    return (int32_t)(intptr_t)(item->what);
}
#pragma GCC diagnostic pop

static int32_t compareItems( const struct CS_ListItem *left, const struct CS_ListItem *right ) {
...
bool test_list(void) {
    ...
    if( testList ) {
        ...
        for( int32_t i = 0; i < 10; ++i ) {
            ...
            if( item ) CS_FAIL_ON_FALSE( i == *(int32_t*)item->what, CS_tempBuffSnprintf( 64, "Checking value of %d", i ), "Got %d", *(int32_t*)item->what );
            int32_t negativeIndex = -1 - i;
            int32_t valueThatShouldBeThere = 9 - i;
            ...
            if( item ) CS_FAIL_ON_FALSE( valueThatShouldBeThere == *(int32_t*)item->what, CS_tempBuffSnprintf( 64, "Checking value of %d", negativeIndex), "Wanted %d Got %d", valueThatShouldBeThere, *(int32_t*)item->what );
        }
    ...
    if( testList ) {
        ...
        GET_AND_CHECK(1,1);
        if( item ) CS_listPushBefore( testList, item, (void*)(int32_t)99, 0 );
        //0,99,1,2,3,4
        VERIFY_LENGTH(6);
        GET_AND_CHECK(1,99);
        GET_AND_CHECK(2,1);
        if( item ) CS_listPushAfter( testList, item, (void*)(int32_t)35, 0 );
        //0,99,1,35,2,3,4
        VERIFY_LENGTH(7);
        GET_AND_CHECK(3,35);
        GET_AND_CHECK(2,1);
        ...
        GET_AND_CHECK(-4,35);
        GET_AND_CHECK(-2,3);
        if( item ) CS_listPushBefore( testList, item, (void*)(int32_t)12, 0 );
        //0,99,1,35,2,12,3,4
        VERIFY_LENGTH(8);
        GET_AND_CHECK(-3,12);
        GET_AND_CHECK(-1,4);
        if( item ) CS_listPushAfter( testList, item, (void*)(int32_t)17, 0 );
        //0,99,1,35,2,12,3,4,17
        VERIFY_LENGTH(9);
        GET_AND_CHECK(-1,17);
        GET_AND_CHECK(0,0);
        if( item ) CS_listPushBefore( testList, item, (void*)(int32_t) 55, 0 );
        //55,0,99,1,35,2,12,3,4,17
        VERIFY_LENGTH(10);
        item = CS_listGetByIndex( testList, 0 );
        GET_AND_CHECK(0,55);
        if( item ) CS_listPushAfter( testList, item, (void*)(int32_t)31, 0 );
        ...
        GET_AND_CHECK(-1,17);
        if( item ) CS_listPushBefore( testList, item, (void*)(int32_t)11, 0 );
        //55,31,0,99,1,35,2,12,3,4,11,17
        VERIFY_LENGTH(12);
        GET_AND_CHECK(-2,11);
        GET_AND_CHECK(-1,17);

        int32_t values[] =       {55,31, 0,99, 1,35, 2,12, 3, 4,11,17};
        int32_t valuesSorted[] = { 0, 1, 2, 3, 4,11,12,17,31,35,55,99};
        item = CS_listGetHead( testList );
        for( int32_t i = 0; i < ARRAY_SIZE(values); ++i ) {
        ...
        for( int32_t i = 0; i < ARRAY_SIZE( valuesSorted ); ++i ) {
