...
bool test_stack() {
    ...
    struct CS_Stack *t0 = CS_stackAllocPointer(STACK_SIZE);
    ...
    if( t0 ) {
        for( i = 0; i < STACK_SIZE * 3; ++i ) {
            ...
            CS_FAIL_ON_TRUE( CS_stackPushPointer( t0, __randy[i] ), __temp, "Failed to push %p", __randy[ i ] );
        }
         for( i = 0; i < STACK_SIZE * 3; ++i ) {
            snprintf( __temp, 1024, "Pop %p", __randy[ STACK_SIZE * 3 - i - 1 ] );
            void *popped = CS_stackPopPointer( t0 );
