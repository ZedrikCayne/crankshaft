...
#define FUZZ_BUFF_SIZE (FUZZ_BUFF_COUNT * sizeof(int32_t))

bool test_base64(void) {
    //Tests go here:
    int32_t outLength;
    int32_t outLength2;
    for( int32_t i = 0; i < sizeof( testStrings ) / sizeof( testStrings[0] ); ++i ) {
    ...
    int32_t *fuzzBuff = CS_alloc( FUZZ_BUFF_SIZE );
    ...
    if( fuzzBuff ) {
        ...
        for( int32_t j = 0; j < FUZZ_BUFF_COUNT; ++j ) {
            fuzzBuff[ j ] = CS_testRand();
        }
        for( int32_t i = 0; i < 50; ++i ) {
            for( int32_t j = CS_testRandMax( 4 ); j < FUZZ_BUFF_COUNT; j += CS_testRandMax( 4 ) ) {
                fuzzBuff[ j ] = CS_testRand();
            }
            int32_t inputSize = CS_testRandMax( FUZZ_BUFF_SIZE );
            int32_t encodedSize;
            int32_t decodedSize;
